...
def labradhdf5_to_ndarray(dir_path, file_num, file_name):
    ...
    # Load hdf5 file
    f_name = (
        "0" * (5 - len(str(file_num))) + str(file_num) + " - " + file_name + ".hdf5"
    )
    ...
    indep_keys = sorted(
        [
            str(x)
            for x in list(attrs.keys())
            if str(x).startswith("Independent") and str(x).endswith("label")
        ]
    )
    dep_keys = sorted(
        [
            str(x)
            for x in list(attrs.keys())
            if str(x).startswith("Dependent") and str(x).endswith("label")
        ]
    )
...
def labrad_hdf5_get_parameters(dir_path, file_num, file_name):
    ...
    param_ukeys = sorted(
        [str(x) for x in list(attrs.keys()) if str(x).startswith("Param")]
    )
